...
namespace="${K8S_LOGS_NAMESPACE:-opamp-logs-poc}"
...
kubectl apply -f "$root/lab/configs/kubernetes/otelcol-logs-opamp-daemonset.yaml"
...
kubectl -n "$namespace" create configmap otelcol-logs-opamp-env \
  --from-literal=opamp-server-ws-endpoint="wss://$opamp_ws_host:4320/v1/opamp" \
  --dry-run=client -o yaml | kubectl apply -f -

kubectl -n "$namespace" rollout restart daemonset/otelcol-logs-opamp
kubectl -n "$namespace" rollout restart deployment/synthetic-k8s-logs
kubectl -n "$namespace" rollout status daemonset/otelcol-logs-opamp --timeout=240s
kubectl -n "$namespace" rollout status deployment/synthetic-k8s-logs --timeout=240s
