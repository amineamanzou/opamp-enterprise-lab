...
Record the pricing model separately from fleet-management capability. In this lab, Fleet OTel-only pricing is treated as part of Elastic Cloud usage, ingest, retention, and subscription tier unless the authenticated account shows a separate collector-management charge.
...
- pricing or plan limits observed in the account, if any.
